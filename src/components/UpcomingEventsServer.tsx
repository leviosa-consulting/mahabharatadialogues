...
interface RetreatData {
  ...
  city?: string
  mapUrl?: string
  day1: { date: string; dayName: string }
  day2?: { date: string; dayName: string }
  day3?: { date: string; dayName: string }
  slug?: string
  bookingUrl: string
  coverImage: string
}
...
export default function UpcomingEventsServer() {
  ...
  useEffect(() => {
    async function fetchUpcomingItems() {
      try {
        ...
        futureDate.setDate(now.getDate() + 300)
        ...
        if (retreatsData.success && retreatsData.data) {
          retreatsData.data.forEach((retreat: RetreatData) => {
            ...
            if (retreatDate >= now && retreatDate <= futureDate) {
        ...
        setFeaturedItem(allItems.length > 0 ? allItems[0] : null)
  ...
  if (loading) {
    return (
      <div
        className="w-full relative mb-30 min-h-[400px] flex items-center justify-center"
        style={{
          backgroundImage: `
            linear-gradient(rgba(29, 92, 117, 0.5), rgba(29, 92, 117, 0.5)),
